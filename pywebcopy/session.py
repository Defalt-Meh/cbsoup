...
# Hoisted constants (avoid rebuilding strings/dicts repeatedly)
...
def default_headers(**kwargs):
    ...
    """
    if kwargs:
        d = _DEFAULT_HEADERS_BASE.copy()
        d.update(kwargs)
        return CaseInsensitiveDict(d)
    return CaseInsensitiveDict(_DEFAULT_HEADERS_BASE)
...
def check_connection(host=None, port=None, timeout=None):
    ...
    port = port or 53
    try:
        # Faster & simpler than manual socket() + connect()
...
class Session(requests.Session):
    ...
    def __init__(self):
        ...
        # Micro-caches for the hot path
        self._ua_cached = self.headers.get('User-Agent', '*')
    ...
    def set_follow_robots_txt(self, b):
        ...
        # Lazy formatting (no string build if DEBUG disabled)
    ...
    def load_rules_from_url(self, robots_url, timeout=None):
        ...
        try:
            # Build a PreparedRequest and bypass our overridden send()
        ...
        _parser.modified()  # initiate start time for delays
        return _parser
    ...
    def is_allowed(self, request, timeout=None):
        # Cache UA (update only if caller changed headers)
        ua = request.headers.get('User-Agent', '*')
        if ua != self._ua_cached:
            self._ua_cached = ua

        s, n, p, q, f = urlsplit(request.url)

        if n in self.domain_blacklist:
            self.logger.error("Blocking request to a blacklisted domain: %r", n)
            return False
        ...
        # Per-host rules fast path
        if n == self._last_host and self._last_rules is not None:
            access_rules = self._last_rules
        else:
            robots_url = s + '://' + n + '/robots.txt'
            access_rules = self.robots_registry.get(robots_url)
            if access_rules is None:
                access_rules = self.load_rules_from_url(robots_url, timeout)
            self._last_host = n
            self._last_rules = access_rules

        if access_rules is None:  # error - everybody welcome
            return True

        allowed = access_rules.can_fetch(self._ua_cached, request.url)
        if not allowed:
            return False
        request_rate = access_rules.request_rate(self._ua_cached)
        ...
        # Original behavior: sleep only if delay is an integer and window not elapsed
